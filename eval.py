...
def evaluate_one_epoch():
    ...
    for batch_idx, batch_data_label in enumerate(TEST_DATALOADER):
        ...
        # for debug, check the prob
        for b in range(8):
            k1=end_points['vote_label_mask'][b,:]
            k2=end_points['prob'][b,:,:] # 1 * num_seed
            k3=k1[ end_points['seed_inds'][b,:].long()] # num_seed,
            id=np.where(k3.cpu())
            posi_prob=k2[0,id][0,:]
            id2=np.where(k3.cpu()==0)
            neg_prob=k2[0,id2][0,:]
            print('batch{}'.format(b))
            print('pos number:{}'.format(posi_prob.shape[0]))
            print('pos > 0.5 : {}'.format(sum((posi_prob>0.5))/posi_prob.shape[0]))
            print('pos > 0.7 : {}'.format(sum((posi_prob>0.7))/posi_prob.shape[0]))
            print('pos > 0.9 : {}'.format(sum((posi_prob>0.9))/posi_prob.shape[0]))

            print('neg number:{}'.format(neg_prob.shape[0]))
            print('neg < 0.5 : {}'.format(sum((neg_prob<0.5))/neg_prob.shape[0]))
            print('neg < 0.3 : {}'.format(sum((neg_prob<0.3))/neg_prob.shape[0]))
            print('neg < 0.1 : {}'.format(sum((neg_prob<0.1))/neg_prob.shape[0]))

...
def read_aggregation(filename):
    ...
    with open(filename) as f:
        data = json.load(f)
        num_objects = len(data['segGroups']) # store in a dict, whose element is a dict
        for i in range(num_objects):
            object_id = data['segGroups'][i]['objectId'] + 1 # instance ids should be 1-indexed, start form 1
...
def read_segmentation(filename):
    ...
    with open(filename) as f:
        data = json.load(f)
        num_verts = len(data['segIndices']) # the number of points ? 
        for i in range(num_verts):
            seg_id = data['segIndices'][i]
            if seg_id in seg_to_verts:
                seg_to_verts[seg_id].append(i) # put the points belong to the same seg in one place
            else:
                seg_to_verts[seg_id] = [i] 
    return seg_to_verts, num_verts # seg_to_verts: idicating that which segs the points belong to
...
def export(mesh_file, agg_file, seg_file, meta_file, label_map_file, output_file=None):
    ...
    # object_id_to_segs: object id : instance segs
    object_id_to_segs, label_to_segs = read_aggregation(agg_file) # label_to_segs: (the same label: segs),
    ...
    for label, segs in label_to_segs.items():
        ...
        for seg in segs:
            ...
            label_ids[verts] = label_id # assign the points to the corresponding label
    instance_ids = np.zeros(shape=(num_verts), dtype=np.uint32) # 0: unannotated
    num_instances = len(np.unique(list(object_id_to_segs.keys())))
    for object_id, segs in object_id_to_segs.items():
        for seg in segs:
            verts = seg_to_verts[seg]
            instance_ids[verts] = object_id # indicating that which instance the points belong to 
            if object_id not in object_id_to_label_id:
                object_id_to_label_id[object_id] = label_ids[verts][0] # to assign each object a label
...
def main():
    ...
    # parser.add_argument('--output_file', required=True, help='output file') # comment for debug

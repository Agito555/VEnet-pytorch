...
import setproctitle
setproctitle.setproctitle("lzj:sv2_1029_newschedule")
...
parser.add_argument('--seed', type=int,default=1029, help='Dump results.')
parser.add_argument('--woptimizer', action='store_true', help='Dump results.')
...
TRAIN_DATALOADER = DataLoader(TRAIN_DATASET, batch_size=BATCH_SIZE,
    shuffle=True, num_workers=4, worker_init_fn=my_worker_init_fn,drop_last=True) # for more stable training
...
if FLAGS.woptimizer:
    optimizer = optim.AdamW(net.parameters(), lr=BASE_LEARNING_RATE, weight_decay=FLAGS.weight_decay)
else:
...
def train_one_epoch():
    ...
    for batch_idx, batch_data_label in enumerate(TRAIN_DATALOADER):
        ...
        batch_interval = 10
        if (batch_idx+1) % batch_interval == 0: # log in every 10 iteration, sum loss and average

...
class PointnetSAModuleVotes(nn.Module):
    ...
    def __init__(
        ...
        if not self.new_mlp:
            ...
            if not amlp:
                self.mlp_module = pt_utils.CMLP2(mlp_spec,mlp2,bn=bn)
            else:
                self.mlp_module = pt_utils.AMLP(mlp_spec,mlp2,bn=bn)
        else:
            self.mlp_module = pt_utils.CMLP(mlp_spec,bn=bn)
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        xyz_flipped = xyz.transpose(1, 2).contiguous() # (B, N, 3) -> (B, 3, N)
        if inds is None:
        ...
        new_xyz = pointnet2_utils.gather_operation(
            xyz_flipped, inds
        ).transpose(1, 2).contiguous() if self.npoint is not None else None  # (B, 3, npoint) -> (B, npoint, 3)
        ...
        new_features = self.mlp_module(
            grouped_features
        )  # (B, mlp[-1], npoint, nsample) or CMLP:(B, mlp[-1], npoint, 1) 
        if not self.new_mlp:
            if self.pooling == 'max':
                new_features = F.max_pool2d(
                    new_features, kernel_size=[1, new_features.size(3)]
